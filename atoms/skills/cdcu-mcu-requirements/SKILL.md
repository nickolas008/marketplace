...
用户提供 CSV 文件路径
    ...
    → 使用 xlsx 将转换结果写入「系统需求」列
    → 输出：保留原始列的基础上，「系统需求」列已填充的 CSV 文件
```
...
用户提供图片（PNG / JPG 等）
    ...
    → 将 OCR 原始文字保存为 requirements_input.csv
          列：「需求描述」（禁止任何修改）
    → 读取 requirements_input.csv，对每行调用 Agent 的功能1→2→3→4
    → 生成 requirements_output.csv：
