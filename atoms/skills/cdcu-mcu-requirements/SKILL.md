...
用户提供 CSV 文件路径
    ...
    → 使用 xlsx 将转换结果写入「系统需求」列，保存到输出目录
    → 输出：输出目录内「系统需求」列已填充的 CSV 文件
```
...
```
用户提供图片（PNG / JPG 等）
    → 使用 paddleocr-doc-parsing 提取文字，把所有文字合并成一个string
    → 将合并后的那个string写入 requirements_input.csv中一行/条的「需求描述」列值 到输出目录。注意string作为一行内容，如果包含了换行符，请用\n替换，避免变成多行。
    → 读取输出目录中的 requirements_input.csv，调用 Agent 的功能1→2→3→4
    → 生成 requirements_output.csv 到该子目录：
          列1：需求描述（直接复制自requirements_input.csv，不得修改）
          列2：系统需求（Agent 转换结果）
```
